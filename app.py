...
def video():
    ...
    if isValidLink == True:
        ...
        info = { "title" : video.title, "duration" : seconds_to_duration(video.length), "thumbnailSrc": thumbNail_src }
...
def get_streams():
    ...
    if request.method == "GET":
        streams = [ { "res" : stream.resolution, "dl_link" : stream.url, "filesize" : round(stream.filesize_mb) } for stream in streamQuery ]
        return { "list" : streams }
